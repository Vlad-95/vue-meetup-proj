...
export const addDays = (date, days) =>
  makeDate(date.getTime() + days * MILLISECONDS_IN_DAY);
...
export const addMonths = (date, months) =>
  makeDate(makeDate(date).setUTCMonth(date.getUTCMonth() + months));
...
export const getFirstDateOfMonth = (date) =>
  makeDate(makeDate(makeDate(date).setUTCDate(1)).setUTCHours(0, 0, 0, 0));
...
export const getLastDateOfMonth = (date) =>
  addDays(getFirstDateOfMonth(addMonths(date, 1)), -1);
